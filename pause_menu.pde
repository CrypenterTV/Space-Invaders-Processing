class PauseMenu extends Menu {
  ...
  void initButtons() {
    ...
    // Bouton de reprise de la partie en cours.
    ...
    addButton(new Button(new PVector((width - int(1.5 * _menuWidth / 3)) / 2, height / 4 + 5 * _menuHeight / 9), int(1.5 * _menuWidth / 3), _menuHeight / 9, "Sauvegarder", COLOR_TEXT_BUTTON, new ButtonAction() {
      ...
      void onClick(int mb) {
        ...
        selectOutput("Exporter la partie en cours : ", "exportLevel"); // Fonction processing pour ouvrir une fenêtre de sélection de fichier windows (cf. documentation).
